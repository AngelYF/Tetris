...
public class Game extends Canvas implements Runnable {
    ...
    private final ViewMainWindow window;
    private final int xPositionCanvas;
    private final int yPositionCanvas;
    private final Piece piece;
    private final Piece nextPiece;
    private final Piece reservationPiece;
    private final int[][] pieceMatrix = new int[ROW][COL];
    private final int[][] nextMatrix = new int[ROW][COL];
    private final int[][] reservationMatrix = new int[ROW][COL];
    private final int[][] trashMatrix = new int[ROW][COL];
    private final Color[][] colorMatrix = new Color[ROW][COL];
    ...
    public void move(int direction) {
        ...
        if (!this.pieceInGame || this.inPause) {
            return;
        }
    ...
    public void update(Graphics graphics) {
        ...
        graphicGame.clearRect(0, 0, getWidth(), getHeight());

        panel(graphicGame);
        subpanel(graphicGame);

        graphics.drawImage(bufferedImageGame, 0, 0, this);
    }
                ...
                if (!pieceInGame) {
                    ...
                    try {
                        ...
                        Thread.sleep((1000 - (level * 75L)));
                            ...
                            InterruptedException e) { // When the piece falls, it calls the interrupted for direct pass to the next one.
                        if (this.dificult.equals("normal")) {
                            // Delay.
                            try {
                                Thread.sleep((1000 - (level * 75L)));
    ...
    private void createBuffers() {
        if (bufferedImageGame == null) {
            bufferedImageGame = (BufferedImage) createImage(ViewMainWindow.WIDTH, ViewMainWindow.HIGH);
            graphicGame = (Graphics2D) bufferedImageGame.getGraphics();
        }
    }
    ...
    public void panel(Graphics graphics) {
        ...
        if (this.newPiece || !this.firstReservation) {
        ...
        if (!this.firstReservation || this.reservation) {
            graphics.setColor(Color.BLACK);
            graphics.fillRect(DISTANCE_SUBPANEL_X + WIDTH_SUBPANEL / 2 - (int) (2.14f * SIZE),
        ...
        for (int i = 0; i < ROW; i++) {
            for (int j = 0; j < COL; j++) {
                ...
                if (reservationMatrix[i][j] == 1 && this.firstReservation) {
